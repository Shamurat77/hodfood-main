  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
        ...
        <Typography fontWeight={900} fontSize="1.5rem">Xaridlar tarixi</Typography>
        <Chip label={filtered.length} size="small" sx={{ fontWeight: 800, ml: 0.5 }} />
      </Box>
      ...
      <Paper variant="outlined" sx={{ p: 2, borderRadius: 3, mb: 2, display: 'flex', gap: 2, flexWrap: 'wrap', alignItems: 'center' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="body2" fontWeight={700} color="text.secondary">Filter:</Typography>
        </Box>
        <FormControl size="small" sx={{ minWidth: 150 }}>
          <InputLabel>Filial</InputLabel>
          <Select value={filterBranch} label="Filial" onChange={(e) => setFilterBranch(e.target.value)} sx={{ borderRadius: 2 }}>
        ...
        <FormControl size="small" sx={{ minWidth: 160 }}>
          ...
          <Select value={filterStatus} label="Holat" onChange={(e) => setFilterStatus(e.target.value)} sx={{ borderRadius: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
          {filtered.map((order) => (
            <Paper key={order.id} variant="outlined" sx={{ borderRadius: 3, overflow: 'hidden' }}>
              <Box sx={{ px: 2.5, py: 1.5, bgcolor: 'grey.50', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
                  <Typography fontWeight={800}>#{order.id}</Typography>
                  <Chip label={order.branch} size="small" color="warning" variant="outlined" sx={{ height: 22, fontSize: '0.7rem', fontWeight: 700 }} />
